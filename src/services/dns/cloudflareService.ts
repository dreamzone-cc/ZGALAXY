...
export class CloudflareService {
  ...
  public static async saveConfig(updated: Partial<CloudflareConfig>): Promise<CloudflareConfig> {
    ...
    // Stale cached ids are invalidated whenever the names they refer to change,
    // otherwise syncs keep updating the old zone/record.
    if (updated.zoneName !== undefined && updated.zoneName !== current.zoneName) {
      merged.zoneId = '';
      merged.recordId = '';
    }
    if (updated.recordName !== undefined && updated.recordName !== current.recordName) {
      merged.recordId = '';
    }
  ...
  public static async syncDNS(force: boolean = false): Promise<{ synced: boolean; message: string; ip: string }> {
    ...
    // 1. Detect public IP — never publish a private/LAN address to public DNS.
    const addrs = await NetworkService.getNetworkAddresses();
    const currentPublicIp = addrs.external.ip4 || '';
    ...
    try {
      ...
      if (targetZoneId && !targetRecordId && cfg.recordName) {
        ...
        // Match on name AND type so a same-named CNAME/AAAA never hijacks an A record.
        const recordType = cfg.recordType || 'A';
        const matchedRecord = records.find(
          (r) => r.name.toLowerCase() === fullDomainName.toLowerCase() && r.type.toUpperCase() === recordType.toUpperCase()
        );
        if (matchedRecord) {
