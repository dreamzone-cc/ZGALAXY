...
// Sanitize config before returning to any client: never expose the raw apiToken.
const sanitizeConfig = (cfg: any): any => {
  const { apiToken, ...rest } = cfg;
  return {
    ...rest,
    apiTokenMasked: maskToken(apiToken || ''),
    hasApiToken: Boolean(apiToken && apiToken.trim().length > 0),
  };
};
...
cloudflareRouter.get('/config', async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: sanitizeConfig(cfg) });
  } catch (err) {
    next(err);
  }
});
...
cloudflareRouter.post('/config', requireRole('ADMIN', 'OPERATOR'), async (req, res, next) => {
  try {
    const { enabled, mode, apiToken, zoneId, zoneName, recordId, recordName, recordType, proxied, autoRebuildPlanet } = req.body;

    if (mode !== undefined && !['MANUAL', 'AUTOMATIC'].includes(mode)) {
      return res.status(400).json({ success: false, error: `Invalid mode '${mode}'.` });
    }
    if (recordType !== undefined && !['A', 'AAAA'].includes(recordType)) {
      return res.status(400).json({ success: false, error: `Invalid recordType '${recordType}'.` });
    }
    if (zoneName !== undefined && typeof zoneName !== 'string') {
      return res.status(400).json({ success: false, error: 'zoneName must be a string.' });
    }
    if (recordName !== undefined && typeof recordName !== 'string') {
      return res.status(400).json({ success: false, error: 'recordName must be a string.' });
    }
    if (apiToken !== undefined && typeof apiToken !== 'string') {
      return res.status(400).json({ success: false, error: 'apiToken must be a string.' });
    }
...
cloudflareRouter.post('/sync', requireRole('ADMIN', 'OPERATOR'), async (req, res, next) => {
  try {
...
cloudflareRouter.delete('/logs', requireRole('ADMIN', 'OPERATOR'), async (req, res, next) => {
  try {
