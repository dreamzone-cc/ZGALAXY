<script lang="ts">
	...
	// API base URL: prefer PUBLIC_API_URL (runtime env, served by adapter-node);
	// otherwise same-origin (empty string resolves relative to the console host).
	// Keeps the production build free of hardcoded hostnames/secrets.
	const publicApiUrl: string = env.PUBLIC_API_URL || '';
	let apiUrl = $state(publicApiUrl);
	let apiKey = $state('');
	...
	// Global request tracker for the [ WORKING... ] indicator
	let requestCount = $state(0);
	let isBusy = $derived(requestCount > 0);
	...
	// Never write live secrets into the on-screen log panel.
	function redactSensitive(obj: any): any {
		if (!obj || typeof obj !== 'object') return obj;
		const out: any = Array.isArray(obj) ? [] : {};
		for (const [k, v] of Object.entries(obj)) {
			if (/tokenSecret|apiToken|providerToken|password/i.test(k)) {
				out[k] = v && typeof v === 'string' && v.length > 8 ? `${v.slice(0, 4)}...${v.slice(-4)}` : '********';
	...
	// Reset all dashboard state (used on logout and session expiry) so stale
	// data never flashes to the next login on a shared machine.
	...
	// Mobile-friendly quick navigation between dashboard panels.
	...
	async function apiRequest(endpoint: string, method = 'GET', body: any = null, customHeaders: Record<string, string> = {}) {
		...
		try {
			...
			// Redact secrets and truncate before echoing into the on-screen log
			// (large payloads would otherwise bloat memory and render cost).
			const logged = JSON.stringify(redactSensitive(data));
			logMessage(`[ ZGALAXY API RES ${res.status} ] ${logged.length > 400 ? logged.slice(0, 400) + '...' : logged}`);

			// Centralized expired-session handling: return to login on 401.
			if (res.status === 401 && isAuthenticated && endpoint !== '/api/v1/auth/login') {
				clearAuthState();
				logMessage('[ AUTH ] Session expired or invalid. Please log in again.');
				showCustomAlert('[ SESSION EXPIRED ]', 'Your session has expired. Please authenticate again.');
			}
			return data;
		} catch (err: any) {
	...
	async function handleLogin() {
		...
		if (!loginUsername.trim()) {
			...
			logMessage('[ FAIL ] ' + loginError);
			return;
		}
		if (!loginPassword) {
			loginError = 'Password is required. Enter your member password to continue.';
			logMessage('[ FAIL ] ' + loginError);
			return;
		}
	...
	function handleLogout() {
		...
		clearAuthState();
		logs = ['[ SYSTEM INIT ] ZGalaxy Planet & Moon TUI Console Initialized.'];
	}
	...
	async function handleFetchCloudflareZones() {
		...
		const customHeader: Record<string, string> = cfApiToken ? { 'x-cloudflare-token': cfApiToken.trim() } : {};
	...
	async function handleCreateMoon() {
		...
		if (!endpointsArr.length || endpointsArr.some((e) => !e)) {
			showCustomAlert('[ INVALID INPUT ]', 'Enter at least one endpoint (e.g. 10.0.0.1/9993).');
			return;
		}
		...
		if (res && res.success) {
			...
			logMessage(`[ FAIL ] Moon creation failed: ${res?.error || 'Unknown error'}`);
			showCustomAlert('[ MOON CREATION FAILED ]', res?.error || 'Failed to create Moon. Check the engine logs.');
		}
	}
	...
	async function loadDashboardData() {
		...
		// Network address detection can hit external IP providers and is the
		// slowest call; load it lazily so it never blocks the dashboard render.
		void autoDetectAddresses();
	}
		...
		(async () => {
			if (typeof window !== 'undefined' && window.location.hostname) {
				if (publicApiUrl) {
					...
					// Same-origin by default (reverse-proxy friendly). Only fall back to
					// hostname:3000 for local dev when same-origin is unreachable.
					apiUrl = '';
					await checkHealth();
					if (systemStatusClass !== 'tui-badge-ok') {
...
<main>
	...
	{#if dialogOpen}
		<div class="tui-modal-overlay" role="dialog" aria-modal="true" aria-label={dialogTitle} tabindex="-1" onkeydown={(e) => {
			...
			<div class="tui-box">
				...
				<div class="tui-body">
					<form onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
						...
						{#if loginError}
							<div class="tui-badge tui-badge-error" style="display: block; margin-bottom: 14px; padding: 6px 10px;" aria-live="polite">
								[ ERROR: {loginError} ]
							</div>
						{/if}
						...
						<div style="margin-bottom: 12px;">
							...
							<input id="loginUsernameInput" type="text" class="tui-input" bind:value={loginUsername} placeholder="admin" autocomplete="username" spellcheck="false" style="margin-top: 4px;" />
						</div>

						<div style="margin-bottom: 16px;">
							<label for="loginPasswordInput" style="color: var(--accent-gold);">[ MEMBER PASSWORD ]</label>
							<input id="loginPasswordInput" type="password" class="tui-input" bind:value={loginPassword} placeholder="••••••••" autocomplete="current-password" style="margin-top: 4px;" />
						</div>

						<button class="tui-btn tui-btn-success" onclick={handleLogin} disabled={isBusy} style="width: 100%; text-align: center; padding: 10px;">
							[ {isBusy ? 'AUTHENTICATING...' : 'AUTHENTICATE & ENTER CONSOLE'} ]
						</button>

						<div style="margin-top: 14px; font-size: 11px; color: var(--text-muted); text-align: center;">
							Default username: <span style="color: var(--accent-gold);">admin</span> | Password: <span style="color: var(--accent-gold);">(set via environment / first login policy)</span>
						</div>
					</form>

					<div style="margin-top: 16px; padding-top: 12px; border-top: 1px dashed var(--border-color); font-size: 11px; color: var(--text-muted); text-align: center;">
	...
	{:else}
			...
			<div class="tui-body">
				...
				<div style="margin-top: 12px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;">
					<div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
						...
						<span class="tui-badge {systemStatusClass}" aria-live="polite">{systemStatus}</span>
						{#if isBusy}
							<span class="tui-badge tui-badge-warn" aria-live="polite">[ WORKING... ]</span>
						{/if}
					</div>
				...
				<div style="margin-top: 12px;">
					<select class="tui-input" aria-label="Quick navigation to dashboard panels" onchange={(e) => { jumpToPanel((e.target as HTMLSelectElement).value); (e.target as HTMLSelectElement).value = ''; }}>
						...
						{#if currentUserRole === 'ADMIN'}
							<option value="panel-members">MEMBERS</option>
						{/if}
						<option value="panel-network">NETWORK & DOMAINS</option>
						<option value="panel-ddns">DDNS ENGINE</option>
						<option value="panel-planet">PLANET & MOON</option>
						<option value="panel-identity">IDENTITY & BACKUP</option>
						<option value="panel-logs">CONSOLE LOG</option>
					</select>
				</div>
			</div>
		</div>
		...
		<div class="tui-box" id="panel-federation">
			<div class="tui-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
									...
									<tr>
										...
										<td>
											<div style="display: flex; gap: 4px;">
												<button class="tui-btn" onclick={() => showCustomAlert('[ TOKEN SECRET (MASKED) ]', tok.hasSecret ? tok.tokenSecretMasked : 'Secret is only shown once at creation.')} style="padding: 2px 6px; font-size: 11px;">[ VIEW SECRET ]</button>
												{#if currentUserRole === 'ADMIN' || currentUserRole === 'OPERATOR'}
		...
		<div class="tui-box" id="panel-cluster">
			<div class="tui-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
		...
		<div class="tui-box" id="panel-cloudflare">
			<div class="tui-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
		...
		{#if currentUserRole === 'ADMIN'}
			<div class="tui-box" id="panel-members">
		...
		<div class="tui-box" id="panel-network">
		...
		<div class="tui-box" id="panel-ddns">
		...
		<div class="grid-2" id="panel-planet">
			...
			<div class="tui-box">
				...
				<div class="tui-body">
					...
					{:else}
						<div class="tui-table-wrap tui-table-wrap--fit">
						<table class="tui-table">
							<thead>
								<tr>
									<th>FILE NAME</th>
		...
		<!-- Console Output Box -->
		<div class="tui-box" id="panel-logs">
